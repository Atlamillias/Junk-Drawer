...
import contextlib
import collections
from typing import overload, Any, Mapping, MutableMapping, Callable, TypeVar, ClassVar, Protocol
from abstract import Property
...
def subscribable_field(fget: Any = None, /, *args, **kwargs) -> Any:
    ...
    Args:
        - fget: Mirrors `property().fget`. Defaults to None.

        - fset: Mirrors `property().fset`. Defaults to None.

        - fdel: Mirrors `property().fdel`. Defaults to None.

        - doc: Mirrors `property().__doc__`. Defaults to None.

        - on_set: If True, allows subscribers to register callbacks to the
        member's `EventType.ON_SET` events, firing when the target attribute
        is set. Defaults to None.

        - on_del: If True, allows subscribers to register callbacks to the
...
def subscribable_method(*args, **kwargs) -> Any:  # type: ignore
    ...
    Args:
        - fn: The method in which subscriptions will be enabled for.

        - on_pre_call: If True, allows subscribers to register callbacks to the
        method's `EventType.ON_PRE_CALL` events. Defaults to None.

        - on_post_call: If True, allows subscribers to register callbacks to the
...
class _Subscribable(Protocol):
    ...
    # The use of a cache prevents conflicts between instance state
    # and states managed by the framework. Additionally, it allows for
...
class Subscribable(_Subscribable):
    ...
    def notify(self, field: str, event_flags: EventFlag | int):
        ...
        Args:
            - field: The name of a member that supports subscriptions.

            - event_flags: Supported event type(s) as a bitmask of event flags.
    ...
    def subscribe(self, field: str, event_flags: EventFlag | int, callback: EventCallbackT) -> EventCallbackT:
        ...
        Args:
            ...
            - callback: A callable that accepts 3 positional arguments; the
            subscribable object, the member name, and the flag representing the
            type of event that triggered the callback.


        The event types a member supports depends on the flags that were set
        when it was defined.
    ...
    def unsubscribe(self, field: str, event_flags: EventFlag | int, callback: EventCallback):
        ...
        Args:
            ...
            - callback: The callback that was registered
        """
        mutex  = self.__mutex__
        events = self.__events__
    ...
    def unsubscribe_all(self, field: str | None = None, event_flags: EventFlag | int = 0):
        ...
        Args:
            - field: The name of a member that supports subscriptions. If None,
