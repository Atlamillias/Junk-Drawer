...
class itersafe(Generic[T]):
    """A thread-safe equivelent to the `iter` built-in function."""
    __slots__ = ('_iterator', '_next', '_aquire', '_release')

    __DEFAULT = object()

    def __init__(self, iterable: Iterable[T], /):
        lock = threading.Lock()
        self._aquire = lock.acquire
    ...
    def __next__(self, /) -> T: ...
    @overload
    def __next__(self, default: _T = ..., /) -> T | _T: ...
    def __next__(self, default = __DEFAULT):
        self._aquire()
        try:
            return self._next()
...
def bytesizeof(obj: Any, memo: set | None = None, /, *, __missing=object()) -> int:
    ...
    tp = type(obj)
    if not is_cclass(tp):  # instance of slotted user-defined class
