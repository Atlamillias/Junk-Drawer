...
Dialogs will have one or more of the following options:
    - title (str, optional): The name of the dialog.

    - parent (Tk, optional): The dialog's root widget. A global
    default parent is shared between all dialogs created from objects
    in this module.

    - initialfile (str, optional): The default value for the dialog's
    file name input field when showing the dialog. *initialfile* is
    updated whenever `.show()` returns a single, non-None result.

    - initialdir (str, optional): The directory opened when the
    `.show` method is called. *initialdir* is updated whenever `.show()`
    returns a non-None result.

    - initialcolor (str, optional): A 7-character hexidecimal color
    ...
    - filetypes (tuple[tuple[str, str], ...], optional): A sequence
    ...
    *.text"), ...]` -- Alternatively, additional 2-tuples can be
    ...
    - defaultextension (str, optional): Appended to the non-None result
    of `.show()` if a file extension is not explicitly given.

    - multiple (bool, optional): If True, the user will be allowed to
    ...
    - mustexist (bool, optional): If True, any user selected path(s)
    must point to an existing file or directory.

"""
from typing import Any, Callable, ClassVar, TypeVar, dataclass_transform, cast, overload
...
class _dialog:
    ...
    def __call__(self, cls: type[T]) -> type[T]:
        ...
        for field in cls.__dataclass_fields__.values():  #type: ignore
            ...
            if not prop:
                fget = utils.create_function(
                    f"{f_name}_getter",
                    ("self",),
                    (fset_body.format(option=f_name),),
                    field.type,
                )
                fset = utils.create_function(
                    f'{f_name}_setter',
                    ("self", "value",),
                    (fset_body.format(option=f_name),),
                    None,
